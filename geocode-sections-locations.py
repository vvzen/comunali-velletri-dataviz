...
df = df.transpose()
df.columns = ["section number", "section address"]
...
for i, place in enumerate(places):
    try:
        current_section_name = place.encode('utf-8')
        ...
        request_url = "{}/json?address={}&key={}".format(
            GOOGLE_API_URL, current_section_address, GOOGLE_API_KEY)
...
df.to_csv(os.path.join("data", "sections_coordinates.csv"), sep=",")
